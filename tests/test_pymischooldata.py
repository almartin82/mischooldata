...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pymischooldata as mi
        assert hasattr(mi, 'fetch_enr')
        assert hasattr(mi, 'fetch_enr_multi')
        assert hasattr(mi, 'tidy_enr')
        assert hasattr(mi, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pymischooldata as mi
        assert hasattr(mi, '__version__')
        assert isinstance(mi.__version__, str)
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_district' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # Michigan should have ~1.4 million students
