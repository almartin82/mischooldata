...
test_that("Michigan DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Michigan DOE URL and verify HTTP 200")
})
...
test_that("Can download Michigan enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Michigan enrollment file", {
  skip_if_offline()
...
test_that("mischooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- mischooldata::get_available_years()
  ...
  tryCatch({
    raw <- mischooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- mischooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- mischooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- mischooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- mischooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- mischooldata:::get_cache_path(2024, "enrollment")
