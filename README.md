...
This package is part of the [State Schooldata Project](https://github.com/almartin82/njschooldata), which provides a simple, consistent interface for accessing state-published school data in Python and R. The original [njschooldata](https://github.com/almartin82/njschooldata) package for New Jersey inspired this effort to cover all 50 states.

Michigan publishes detailed enrollment data through CEPI, but the files are scattered across multiple URLs, use different formats by year, and require significant cleaning. This package handles all of that complexity so you can focus on analysis.
...
All data is sourced directly from the Michigan Center for Educational Performance and Information (CEPI):
- Main portal: https://www.mischooldata.org/
- Data files: https://www.mischooldata.org/student-enrollment-counts-data-files/
...
CEPI suppresses (shows as blank/NA) student counts when:
- Cell counts are less than 10 students for demographic breakdowns
- The suppression is necessary to prevent identification of individual students
...
2. **Charter school proliferation:** Michigan has ~300 charter schools (PSAs - Public School Academies) which are counted as separate districts.

3. **Schools of Choice:** Inter-district enrollment means some students are counted in districts different from their residence.
...
This package is part of a larger effort to provide simple, consistent access to state-published school data across all 50 states. The project started with [njschooldata](https://github.com/almartin82/njschooldata) (New Jersey) and has expanded to cover the entire country.
