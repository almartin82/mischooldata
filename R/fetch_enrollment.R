...
#' Fetch Michigan enrollment data
#'
#' Downloads and processes enrollment data from the Michigan CEPI
#' MI School Data headcount files.
#'
#' @param end_year A school year. Year is the end of the academic year - eg 2023-24
#'   school year is year '2024'. Valid values are 1996-2025.
#' @param tidy If TRUE (default), returns data in long (tidy) format with subgroup
#'   column. If FALSE, returns wide format.
#' @param use_cache If TRUE (default), uses locally cached data when available.
#'   Set to FALSE to force re-download from CEPI.
...
#'
#' # Filter to specific district (Detroit)
#' detroit <- enr_2024 |>
#'   dplyr::filter(district_id == "82015")
#' }
fetch_enr <- function(end_year, tidy = TRUE, use_cache = TRUE) {

  # Validate year - Michigan data available from 1996 to present
  # Based on research: data files exist from 1995-96 to 2024-25
  if (end_year < 1996 || end_year > 2025) {
    stop("end_year must be between 1996 and 2025")
  }
...
fetch_enr_multi <- function(end_years, tidy = TRUE, use_cache = TRUE) {
  ...
  # Validate years
  invalid_years <- end_years[end_years < 1996 | end_years > 2025]
  if (length(invalid_years) > 0) {
    stop(paste("Invalid years:", paste(invalid_years, collapse = ", "),
               "\nend_year must be between 1996 and 2025"))
  }
